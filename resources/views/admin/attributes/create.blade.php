...
@section('title', __('create attributes'))
...
@section('content')
    <div class="col-md-12">
        ...
        <div class="my-2 bg-white border shadow rounded p-4">
            <form action="{{ route('admin-panel.attributes.store') }}" method="POST">
                @csrf
                @method('POST')
                {{-- name --}}
                <div class="form-group">
                    <label for="name">{{ __("Name") }}</label>
                    <input type="text" name="name" id="name" placeholder="{{ __('Write your attribute name') }}" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror">
                    @error('name')
