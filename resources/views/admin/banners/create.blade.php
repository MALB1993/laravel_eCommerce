...
@section('content')
    <div class="col-md-12">
        ...
        <div class="my-2 bg-white border shadow rounded p-4">
            <form action="{{ route('admin-panel.banners.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
                <div class="row">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="priority" id="priority" class="form-control @error('priority') is-invalid @enderror" value="{{ old('priority') }}" dir="auto">
                        @error('priority')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="button_link" id="button_link" class="form-control @error('button_link') is-invalid @enderror" value="{{ old('button_link') }}" dir="auto">
                        @error('button_link')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="button_icon" id="button_icon" class="form-control @error('button_icon') is-invalid @enderror" value="{{ old('button_icon') }}" dir="auto">
                        @error('button_icon')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <textarea name="text" id="text" cols="30" rows="3" class="form-control @error('text') is-invalid @enderror" placeholder="{{ __('Write Description') }}">{{ old('text') }}</textarea>
                        @error('text')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
